  ...
  try {
    ...
    if (configKey === "componentRates") {
      query = query
        ...
        .order("component_order", { ascending: true, nullsFirst: false })
...
app.get("/make-server-c894a9ff/admin/diagnostic/tams360-check", requireAdmin, async (c) => {
  try {
    ...
    const tenantUsers = results.viewUsers.filter((u: any) => u.tenant_id === userProfile.tenant_id);
...
app.get("/make-server-c894a9ff/assets", async (c) => {
  try {
    ...
    // Fetch latest inspection summaries for the assets on this page.
    // This is needed because tams360_assets_v currently exposes latest_ci as CI Health
    // in some cases, while the asset register/list/map should display CI Final.
    const assetIds = (assets || [])
      .map((asset: any) => asset.asset_id)
      .filter(Boolean);

    const latestInspectionByAsset = new Map<string, any>();
    ...
    if (assetIds.length > 0) {
      ...
      if (latestInspectionError) {
        console.error("[Assets] Warning: failed to fetch latest inspection summaries:", latestInspectionError);
      } else {
    ...
    // Calculate remaining life/current value and attach authoritative latest inspection values.
    const enrichedAssets = (assets || []).map((asset: any) => {
      ...
      return {
        ...
        // Keep the full latest inspection object so frontend helpers can resolve consistently.
        latest_inspection: latestInspection,

        // Authoritative values for frontend display.
...
app.post("/make-server-c894a9ff/inspections", async (c) => {
  try {
    ...
    // Use the same tenant source as the working asset routes.
    // Do not use old KV-only tenant data here.
    const { data: userProfile, error: profileError } = await supabase
      .from("tams360_user_profiles_v")
      .select("id, tenant_id, role")
      .eq("id", userData.user.id)
    ...
    if (profileError || !userProfile || !userProfile.tenant_id) {
      console.error("Inspection create profile lookup failed:", profileError);
      return c.json({ error: "User not associated with an organization" }, 403);
    }

    if (!isValidUuid(userProfile.tenant_id)) {
      console.error(`Invalid tenant ID format: ${userProfile.tenant_id}`);
      return c.json({
        error: "Invalid organization/tenant ID format.",
        details: "tenant_id must be a UUID.",
      }, 400);
    }

    const inspection = await c.req.json();

    const { data: rpcResult, error: inspectionError } = await supabase
      .rpc("tams360_mobile_create_inspection", {
    ...
    if (inspectionError) {
      console.error("Error creating inspection via RPC:", inspectionError);
      return c.json({
        success: false,
